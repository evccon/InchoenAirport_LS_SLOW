...
public class AuthWaitView2 extends FrameLayout {
    ...
    public AuthWaitView2(Context context, MultiChannelUIManager manager, Activity activity) {
        super(context);
        flowManager = manager;
        mainActivity = activity;

        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        inflater.inflate(R.layout.page_auth_wait2, this, true);
        ...
        timer = new TimeoutTimer(1000, new TimeoutHandler() {
            ...
            public void run() {
                ...
                if (count == 0) {
                    stopTimer();
                    flowManager.getUIFlowManager(channel).onServerconnctErrorEvent();
    ...
    void initComponents() {
        ...
        tvMain.setText("회원 인증 진행 중 입니다.");
        imageAni = (ImageView) findViewById(R.id.imgloading);
    }
    ...
    protected void onVisibilityChanged(View changedView, int visibility) {
        if (visibility == VISIBLE) {
            ...
            ChargeData chargeData = flowManager.getUIFlowManager(channel).getChargeData();
            if (chargeData.isNomember) tvMain.setVisibility(INVISIBLE);
            else tvMain.setVisibility(VISIBLE);
